...
class MachineTranslation:
    ...
    1. 解码器的计算图只包含单个时间步 (timestep model), 解码采用分时间步的方式(时间步的循环控制写在计算图外面), 即每一个时间步运行解码器得到解码结果

    2. 采用动态图构建模型 (eager execution), 并从头编写训练循环; 在训练时, 使用  @tf.function 包装器 (转换为静态图) 加速计算
...
class Seq2seqModel(Model):
    ...
    def __init__(self,  n_embedding, n_h, target_length,
        ...
        self.decoder = Decoder(n_embedding=n_embedding, n_h=n_h, n_vocab=n_vocab_target, target_length=target_length, dropout_rates=dropout_rates)


        # 损失对象
        self.loss_object = tf.keras.losses.SparseCategoricalCrossentropy(reduction='none', from_logits=True,)
    ...
    # 使用 @tf.function 可以加速计算
    # Tips:
    # [1] 被包装的函数中的变量都采用 tf, 避免使用 numpy 和 list
    # [2] 参与计算的变量的类型应该匹配, 使用 tf.cast 做手动的类型转换
    ...
    def _train_step(self, inputs):
        ...
        N_batch = tf.shape(batch_target_in)[0]
        target_length = tf.shape(batch_target_in)[1]

        loss = tf.constant(0.0)

        probs = tf.zeros((target_length, N_batch, self.n_vocab_target))
        ...
        with tf.GradientTape() as tape:
            ...
            for t in tf.range(target_length):
                ...
                batch_token = tf.expand_dims(batch_target_in[:, t], axis=1)   # shape (N_batch, 1)

                layer_state_list, prob, _ = self.decoder(batch_token=batch_token,
                                           layer_state_list=layer_state_list, training=training)
                # prob shape: (N_batch, n_vocab)

                target_mask, loss_t = self._mask_loss_function(batch_target_out[:, t], prob)

                loss += loss_t

                # probs[:, t, :] = prob.numpy()

                clip = tf.expand_dims(prob, axis=0)  # shape: (N_batch, n_vocab)
                indices = [[t]]
                probs = tf.tensor_scatter_nd_update(probs, indices, clip)

        probs = tf.transpose(probs, perm=[1, 0, 2])  # shape (N_batch, target_length, n_vocab)

        average_loss = (loss / tf.cast(target_length, tf.float32))
    ...
    def _test_step(self, batch_source, target_length):
        ...
        preds = tf.zeros((target_length, N_batch), dtype=tf.int64)

        layer_state_list = self.encoder(batch_source=batch_source, training=training)

        batch_token = tf.ones((N_batch, 1)) * self._start_target  # (N_batch, 1)

        for t in tf.range(target_length):

            layer_state_list, prob, batch_token = self.decoder(batch_token=batch_token,
                                                           layer_state_list=layer_state_list, training=training)

            # probs[:, t, :] = prob.numpy()

            indices = [[t]]
            clip = tf.expand_dims(prob, axis=0)  # shape: (1, N_batch, n_vocab)
            probs = tf.tensor_scatter_nd_update(probs, indices, clip)

            # preds[:, t] = tf.squeeze(batch_token, axis=1).numpy()
            clip = tf.transpose(batch_token, perm=[1, 0])  # shape: (1, N_batch)
            preds = tf.tensor_scatter_nd_update(preds, indices, clip)

        probs = tf.transpose(probs, perm=[1, 0, 2])  # shape (N_batch, target_length, n_vocab)
        preds = tf.transpose(preds, perm=[1, 0])  # shape (N_batch, target_length)
    ...
    def fit(self, epoch_num, train_dataset, valid_dataset, callback_obj=None):
        ...
        for epoch in range(epoch_num):
            ...
            for batch_data in tqdm(train_dataset):
                ...
                # probs shape (N_batch, target_length, n_vocab_target)

                # print('iteration loss: %.2f' % (average_loss,))
            ...
            for batch_data in tqdm(valid_dataset):
                ...
                probs, preds = self._test_step(batch_source, target_length)
                # probs shape (N_batch, target_length, n_vocab_target)
                # preds shape (N_batch, target_length)

                _, loss = self._mask_loss_function(labels, probs)

                average_loss = loss / int(target_length)
...
class Decoder(Layer):
    ...
    def call(self, batch_token, layer_state_list, training=True):

        # batch_token shape (N_batch, 1)

        h0 = layer_state_list[0][0]  # shape: (N_batch, n_h)
        c0 = layer_state_list[0][1]  # shape: (N_batch, n_h)

        batch_token_embbeding = self.embedding_layer(batch_token)
        ...
        prob = self.fc_layer(h_dropout)  # shape (N_batch, n_vocab)

        max_idx = tf.math.argmax(prob, axis=1)  # shape (N_batch, )

        next_batch_token = tf.expand_dims(max_idx, axis=1)  # shape (N_batch, 1)

        layer_state_list[0][0] = h0
        layer_state_list[0][1] = c0

        return layer_state_list, prob, next_batch_token
